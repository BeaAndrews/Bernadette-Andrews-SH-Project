...
/* program parameters */
#define Lx 1
#define Ly 100 // 100 //96
#define Lz 100 // 100 //96				/*32*/ /*240*/ /*100*/
...
double M = 5.0; //0.03;				/*0.05*/ /* need to tweak */
double radius = 40;
...
double zeta = 0.0009;			/*0.0*/ /* activity */
...
double Abulk = 0.01; //1.0; /*determines bulk liquid crystal free energy*/
double L1 = 0.01; //0.001 /*liquid crystal elastic constant*/
double q0 = 0.0; //Pi/16.0; /*pitch - for cholesterics*/
...
#endif
			  phi[i][j][k] = phi0+0.0*(1.0-2.0*drand48());
			}
		}
	}
}
		...
		for (j = 0; j < Ly; j++) {
			for (k = 0; k < Lz; k++) {
				...
				if (R < radius) {
					...
					Qxx[i][j][k]=0.1*(1.0-2.0*drand48());
					Qxy[i][j][k]=0.1*(1.0-2.0*drand48());
					Qxz[i][j][k]=0.1*(1.0-2.0*drand48());
					Qyy[i][j][k]=0.1*(1.0-2.0*drand48());
					Qyz[i][j][k]=0.1*(1.0-2.0*drand48());
		 ...
		 (3.0/2.0*sin(phase2)*sin(phase2)*sin(phase)*sin(phase)-1.0/2.0);
	       ...
	       Qyz[i][j][k]= 3.0*amplitude/2.0*(sin(phase2)*cos(phase2)*sin(phase));
					...
					Qxy[i][j][k]=0.1*(1.0-2.0*drand48());
					Qxz[i][j][k]=0.1*(1.0-2.0*drand48());
					Qyy[i][j][k]=0.3+0.1*(1.0-2.0*drand48());
					Qyz[i][j][k]=0.1*(1.0-2.0*drand48());
	...
	for (i = 0; i < Lx; i++) {
	  for (j = 0; j < Ly; j++) {
	    for (k = 0; k < Lz; k++) {
	      ...
	      DEHxx[i][j][k]+= Abulk*(-(1.0-gammac/3.0+L1*q0*q0/Abulk)*Qxxl+gammac*(Qsqxx-TrQ2/3.0)-gammac*Qxxl*TrQ2);
	      DEHxy[i][j][k]+= Abulk*(-(1.0-gammac/3.0+L1*q0*q0/Abulk)*Qxyl+gammac*Qsqxy-gammac*Qxyl*TrQ2);
	      DEHyy[i][j][k]+= Abulk*(-(1.0-gammac/3.0+L1*q0*q0/Abulk)*Qyyl+gammac*(Qsqyy-TrQ2/3.0)-gammac*Qyyl*TrQ2);
	      DEHxz[i][j][k]+= Abulk*(-(1.0-gammac/3.0+L1*q0*q0/Abulk)*Qxzl+gammac*Qsqxz-gammac*Qxzl*TrQ2);
	      DEHyz[i][j][k]+= Abulk*(-(1.0-gammac/3.0+L1*q0*q0/Abulk)*Qyzl+gammac*Qsqyz-gammac*Qyzl*TrQ2);
...
void plotQ(int n)
{
	...
	sprintf(filename1, "Qtensor0.%d.txt",n);
